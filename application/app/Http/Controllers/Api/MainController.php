...
return response()->json(['error' => 'Secret Key is required']);
...
if($user) {
    $user->detail = strip_tags($user->detail);
}
    ...
    public function detailpage(Request $request)
    {
        ...
        if ($validator->fails()) {
return response()->json(['error' => 'Secret Key is required'], 400);
        ...
        if (!$key) {
return response()->json(['error' => 'Invalid Secret Key !'], 400);
...
return response()->json(['error'=>'Unauthorized Action'], 401);
        ...
        if (!$result) {
return response()->json(['error' => '404 | Course not found !'], 400);
...
if($user) {
    ...
    public function becomeaninstructor(Request $request)
    {
        ...
        if (!$users->isEmpty()) {
            ...
            return response()->json(['error' => 'Already Requested !'], 401);
        } else {
    ...
    public function courseprogress(Request $request)
    {
        ...
        $auth = Auth::guard('api')->user();
$progress = "No Course found with request course Id";
...
$paginator = Course::where('status', 1)->with('include')->with('whatlearns')->with('review')->paginate($page);
