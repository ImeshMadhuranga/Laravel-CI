...
Route::get('email/verify', 'Api\VerificationController@sendCode')->name('verification.code');
Route::post('email/verify', 'Api\VerificationController@verifybyapi')->name('verification.email');
...
Route::post('upload-image', 'Api\Auth\RegisterController@imageUpload');
Route::post('remove-image', 'Api\Auth\RegisterController@imageRemove');
...
Route::get('/logged-out', function () {
    return response()->json(['error' => 'You are not authorized or logged out already.'], 400);
})->name('logged-out');
...
Route::post('instructor/show/profile', 'Api\InstructorApiController@getInstructorProfile');
